...
const server = http.createServer(async (req, res) => {
  ...
  // ---------- POST /generate-posts — manually trigger post generation ----------
  } else if (req.method === 'POST' && req.url === '/generate-posts') {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, message: 'Generation started. Refresh approvals in ~2 minutes.' }));
    runGeneration();

  // ---------- GET /generation-status — last generation info ----------
  } else if (req.method === 'GET' && req.url === '/generation-status') {
    const approvals = loadApprovals();
    const generated = (approvals.posts || []).filter(p => p.generatedAt);
    const last = generated.length
      ? generated.reduce((a, b) => (a.generatedAt > b.generatedAt ? a : b)).generatedAt
      : null;
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({
      running: generationRunning,
      lastGeneratedAt: last,
      pendingCount: (approvals.posts || []).filter(p => p.status === 'pending').length,
      apiKeySet: !!config.ANTHROPIC_API_KEY,
    }));
...
// =====================================================
// Monthly post generation (30th of each month)
// =====================================================
...
async function runGeneration() {
  ...
  try {
    const n = await generatePosts();
    console.log(`[monthly generator] ✓ ${n} posts added to approval queue`);
  } catch (err) {
    console.error(`[monthly generator] ✗ ${err.message}`);
  } finally {
    generationRunning = false;
  }
}

function checkMonthlyGeneration() {
  if (!config.ANTHROPIC_API_KEY) return;
  const now = new Date();
  if (now.getDate() !== 30) return;

  const approvals = loadApprovals();
  const yearMonth = now.toISOString().slice(0, 7); // e.g. "2026-04"
  const alreadyDone = (approvals.posts || []).some(
    p => p.generatedAt && p.generatedAt.startsWith(yearMonth)
  );
  if (alreadyDone) return;

  console.log('[monthly generator] 30th of month — starting post generation…');
  runGeneration();
}
...
// LinkedIn scheduler: every minute + immediate catch-up
const ONE_MINUTE = 60 * 1000;
setInterval(publishDuePosts, ONE_MINUTE);
publishDuePosts();

// Monthly post generator: check once a day + on startup
const ONE_DAY = 24 * 60 * 60 * 1000;
setInterval(checkMonthlyGeneration, ONE_DAY);
checkMonthlyGeneration();
